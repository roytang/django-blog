...
class FeedIncludeNode(template.Node):
    ...
    def render(self, context):
        ...
        items = []
        num_items = min(int(self.num_items), len(feed['entries']))
        for i in range(num_items):
            pub_date = feed['entries'][i].get('updated_parsed')
            if pub_date:
                if len(pub_date) > 3:
                    published = datetime.datetime.fromtimestamp(calendar.timegm(pub_date))
                else:
                    published = datetime.date(pub_date[0], pub_date[1], pub_date[2])
            else:
                published = None
            items.append({ 'title': feed['entries'][i].get('title'),
                           'summary': feed['entries'][i].get('summary'),
                           'link': feed['entries'][i].get('link'),
                           'description': feed['entries'][i].get('subtitle'),
                           'date': published })
